...
public class UIInput : MonoBehaviour
{
	...
	private void OnSelect(bool isSelected)
	{
		...
		if (isSelected)
		{
			...
			float y = position.y;
			position.y = y + label.relativeSize.y;
	...
	private void UpdateLabel()
	{
		...
		if (!label.shrinkToFit)
		{
			if (label.multiLine)
			{
				...
				text = font.WrapText(text2, num / label.cachedTransform.localScale.x, 0, encoding: false, UIFont.SymbolStyle.None);
			}
			else
			{
				...
				string endOfLineThatFits = font2.GetEndOfLineThatFits(text3, num2 / label.cachedTransform.localScale.x, encoding: false, UIFont.SymbolStyle.None);
