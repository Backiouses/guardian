...
public class BTN_ToJoin : MonoBehaviour
{
	private void OnClick()
	{
		...
		if (transform4 == null)
		{
			Vector3 position2 = transform5.localPosition + new Vector3(0f, 35f, 0f);
			GameObject gameObject = transform.Find("LabelIP").gameObject;
			transform4 = CreateLabel(transform.gameObject, gameObject, position2, transform5.rotation, "LabelAuthPass", "Admin Password (Optional)", gameObject.GetComponent<UILabel>().font.dynamicFontSize, gameObject.GetComponent<UILabel>().lineWidth).transform;
