...
public class InstantiateTracker
{
	...
	private class Player
	{
		...
		{
			int resourceData = GetResourceData(gr);
			if (resourceData > -1)
			{
				return Resources[resourceData].KickWorthy();
			}
		...
		{
			int resourceData = GetResourceData(GameResource.Name);
			if (resourceData > -1)
			{
				Resources[resourceData].Reset();
			}
		}
	}
