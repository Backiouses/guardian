...
public class Minimap : MonoBehaviour
{
	...
	public class MinimapIcon
	{
		...
		public static MinimapIcon Create(RectTransform parent, GameObject trackedObject, IconStyle style)
		{
			...
			Vector2 vector2 = (rectTransform.anchorMax = new Vector3(0.5f, 0.5f));
			Vector2 vector4 = (rectTransform.anchorMin = vector2);
		...
		public static MinimapIcon CreateWithRotation(RectTransform parent, GameObject trackedObject, IconStyle style, float pointerDist)
		{
			...
			vector2 = (rectTransform2.anchorMax = rectTransform.anchorMin);
			vector4 = (rectTransform2.anchorMin = vector2);
	...
	private void CreateUnityUI(Texture2D map, int minimapResolution)
	{
		...
		Vector2 vector = (minimapMaskT.anchorMax = Vector2.one);
		Vector2 vector3 = (rectTransform.anchorMin = vector);
		...
		vector = (minimap.anchorMax = new Vector2(0.5f, 0.5f));
		vector3 = (rectTransform2.anchorMin = vector);
	...
	private void CreateUnityUIRT(int minimapResolution)
	{
		...
		vector = (borderT.anchorMax = new Vector2(0.5f, 0.5f));
		vector3 = (rectTransform2.anchorMin = vector);
		...
		vector = (minimap.anchorMax = new Vector2(0.5f, 0.5f));
		vector3 = (rectTransform3.anchorMin = vector);
		minimap.anchoredPosition = Vector2.zero;
		minimap.sizeDelta = minimapMaskT.sizeDelta;
	...
	private void Initialize()
	{
		...
		rect = new Rect(0f, 0f, texture2D.width, texture2D.height);
		borderSprite = UnityEngine.Sprite.Create(texture2D, rect, vector, 100f, 1u, SpriteMeshType.FullRect, new Vector4(5f, 5f, 5f, 5f));
	...
	public void Maximize()
	{
		...
		RectTransform rectTransform = minimapMaskT;
		Vector2 vector2 = (minimapMaskT.anchorMax = new Vector2(0.5f, 0.5f));
		Vector2 vector4 = (rectTransform.anchorMin = vector2);
	...
	private void CheckUserInput()
	{
		if ((int)FengGameManagerMKII.Settings[231] == 1 && RCSettings.GlobalDisableMinimap == 0)
		{
			...
			else
			{
				...
				{
					float num3 = Time.deltaTime * ((Input.GetKey(KeyCode.LeftShift) ? 2f : 0.75f) * lastMinimapOrthoSize);
					lastMinimapCenter.z -= num3;
					flag = true;
				}
				if (Input.GetKey(KeyCode.RightArrow))
				{
					float num4 = Time.deltaTime * ((Input.GetKey(KeyCode.LeftShift) ? 2f : 0.75f) * lastMinimapOrthoSize);
					lastMinimapCenter.x += num4;
					flag = true;
				}
				else if (Input.GetKey(KeyCode.LeftArrow))
				{
					float num5 = Time.deltaTime * ((Input.GetKey(KeyCode.LeftShift) ? 2f : 0.75f) * lastMinimapOrthoSize);
					lastMinimapCenter.x -= num5;
					flag = true;
				}
			}
			if (flag)
