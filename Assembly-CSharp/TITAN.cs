...
public class TITAN : Photon.MonoBehaviour
{
	...
	{
		shouldLookAtTarget = !animName.StartsWith("attack") && !animName.StartsWith("hit") && !animName.StartsWith("eat") && !animName.StartsWith("sit") && !animName.EndsWith("recovery");
		shouldRotateFast = animName.StartsWith("attack") || animName.StartsWith("hit");
	}
	...
	{
		if (TitanChecker.IsAnimationPlayValid(this, info))
		{
			LocalPlayAnimation(aniName);
		}
	}
	...
	{
		if (TitanChecker.IsAnimationSeekedPlayValid(this, info))
		{
			LocalPlayAnimationAt(aniName, normalizedTime);
		}
	}
	...
	{
		if (TitanChecker.IsCrossFadeValid(this, info))
		{
			LocalCrossFade(aniName, time);
		}
	}
	...
	{
		if (!TitanChecker.IsTitanTypeSetValid(this, info) && PhotonNetwork.isMasterClient)
		{
			base.photonView.RPC("netSetAbnormalType", PhotonTargets.OthersBuffered, (int)abnormalType);
			return;
		}
		if (!hasload)
		{
		...
		case 4:
			...
			if (GuardianClient.Properties.OGPunkHair.Value)
			{
		...
		if (abnormalType == TitanClass.Crawler)
		{
			...
			CapsuleCollider component = baseTransform.Find("AABB").gameObject.GetComponent<CapsuleCollider>();
	...
	private void OnDestroy()
	{
		...
		if (gameObject != null)
		{
			gameObject.GetComponent<FengGameManagerMKII>().RemoveTitan(this);
		}
	}
	...
	{
		if (TitanChecker.IsTargetSetValid(this, info) || !PhotonNetwork.isMasterClient)
		{
			if (viewId == -1)
			{
				myHero = null;
	...
	public bool IsGrounded()
	{
		...
		int value = ((LayerMask)((int)(LayerMask)(1 << LayerMask.NameToLayer("EnemyAABB")) | (int)layerMask)).value;
	...
	private GameObject checkIfHitHead(Transform head, float rad)
	{
		...
		{
			if (!player.GetComponent<TITAN_EREN>() && (!player.GetComponent<HERO>() || !player.GetComponent<HERO>().IsInvincible()))
			{
				float num2 = player.GetComponent<CapsuleCollider>().height * 0.5f;
				if (Vector3.Distance(player.transform.position + Vector3.up * num2, head.position + Vector3.up * 1.5f * myLevel) < num + num2)
				{
					return player;
				}
			}
	...
	private string[] GetAttackStrategy()
	{
		...
		{
			switch ((!(Mathf.Abs(between2) < 90f)) ? ((!(between2 > 0f)) ? 3 : 4) : ((between2 > 0f) ? 1 : 2))
			{
			case 2:
				array = ((myDistance < attackDistance * 0.25f) ? ((abnormalType == TitanClass.Punk) ? new string[2] { "attack_kick", "attack_stomp" } : ((abnormalType == TitanClass.Normal) ? new string[2] { "attack_front_ground", "attack_stomp" } : new string[1] { "attack_kick" })) : ((myDistance < attackDistance * 0.5f) ? ((abnormalType == TitanClass.Punk) ? new string[3] { "grab_ground_front_l", "grab_ground_front_l", "attack_abnormal_jump" } : ((abnormalType == TitanClass.Normal) ? new string[3] { "grab_ground_front_l", "grab_ground_front_l", "attack_stomp" } : new string[3] { "grab_ground_front_l", "grab_ground_front_l", "attack_abnormal_jump" })) : ((abnormalType == TitanClass.Punk) ? new string[3] { "attack_combo", "attack_combo", "attack_abnormal_jump" } : ((abnormalType == TitanClass.Normal) ? ((myDifficulty > 0) ? new string[3] { "attack_front_ground", "attack_combo", "attack_combo" } : new string[5] { "attack_front_ground", "attack_front_ground", "attack_front_ground", "attack_front_ground", "attack_combo" }) : new string[1] { "attack_abnormal_jump" }))));
				break;
			case 1:
				array = ((myDistance < attackDistance * 0.25f) ? ((abnormalType == TitanClass.Punk) ? new string[2] { "attack_kick", "attack_stomp" } : ((abnormalType == TitanClass.Normal) ? new string[2] { "attack_front_ground", "attack_stomp" } : new string[1] { "attack_kick" })) : ((myDistance < attackDistance * 0.5f) ? ((abnormalType == TitanClass.Punk) ? new string[3] { "grab_ground_front_r", "grab_ground_front_r", "attack_abnormal_jump" } : ((abnormalType == TitanClass.Normal) ? new string[3] { "grab_ground_front_r", "grab_ground_front_r", "attack_stomp" } : new string[3] { "grab_ground_front_r", "grab_ground_front_r", "attack_abnormal_jump" })) : ((abnormalType == TitanClass.Punk) ? new string[3] { "attack_combo", "attack_combo", "attack_abnormal_jump" } : ((abnormalType == TitanClass.Normal) ? ((myDifficulty > 0) ? new string[3] { "attack_front_ground", "attack_combo", "attack_combo" } : new string[5] { "attack_front_ground", "attack_front_ground", "attack_front_ground", "attack_front_ground", "attack_combo" }) : new string[1] { "attack_abnormal_jump" }))));
				break;
			case 3:
				if (myDistance < attackDistance * 0.5f)
				{
					array = ((abnormalType == TitanClass.Normal) ? new string[1] { "grab_ground_back_l" } : new string[1] { "grab_ground_back_l" });
				}
				break;
			case 4:
				if (myDistance < attackDistance * 0.5f)
				{
					array = ((abnormalType == TitanClass.Normal) ? new string[1] { "grab_ground_back_r" } : new string[1] { "grab_ground_back_r" });
				}
				break;
			}
		}
	...
	{
		if (base.photonView.isMine && (attacker - base.transform.position).sqrMagnitude < 6400f)
		{
			Hit("hit_eren_L", attacker, hitPauseTime);
		}
	}
	...
	{
		if (base.photonView.isMine && !hasDie && (attacker - base.transform.position).sqrMagnitude < 6400f)
		{
			Hit("hit_eren_R", attacker, hitPauseTime);
		}
	}
	...
	{
		if (TitanChecker.IsSoundPlayValid(this, info))
		{
	...
	public void DieHeadBlowFunc(Vector3 attacker, float hitPauseTime)
	{
		...
		if (IN_GAME_MAIN_CAMERA.Gametype == GameType.Multiplayer && base.photonView.isMine)
		{
			...
			Vector3 eulerAngles = neck.rotation.eulerAngles;
			float x = 270f + eulerAngles.x;
			float y = eulerAngles.y;
			gameObject = PhotonNetwork.Instantiate("bloodsplatter", position, Quaternion.Euler(x, y, eulerAngles.z), 0);
		}
		else
		{
			...
			float x2 = 270f + eulerAngles2.x;
			float y2 = eulerAngles2.y;
			gameObject = (GameObject)UnityEngine.Object.Instantiate(original, position2, Quaternion.Euler(x2, y2, eulerAngles2.z));
		}
		gameObject.transform.localScale = base.transform.localScale;
	...
	private void JustEatHero(GameObject target, Transform hand)
	{
		...
		if (IN_GAME_MAIN_CAMERA.Gametype == GameType.Multiplayer && base.photonView.isMine)
		{
			HERO component = target.GetComponent<HERO>();
			if (!component.HasDied())
			{
				component.MarkDead();
				if (nonAI)
				{
					component.photonView.RPC("netDie2", PhotonTargets.All, base.photonView.viewID, base.name);
				}
				else
				{
					component.photonView.RPC("netDie2", PhotonTargets.All, -1, base.name);
				}
			}
		}
		else if (IN_GAME_MAIN_CAMERA.Gametype == GameType.Singleplayer)
	...
	{
		if (TitanChecker.IsRightGrabValid(this, info))
		{
	...
	private void JustHitEye()
	{
		...
		{
			TitanState titanState = state;
			if ((uint)(titanState - 13) <= 1u)
			{
	...
	{
		if (!hasDie && (PhotonView.Find(viewId).gameObject.transform.position - neck.position).sqrMagnitude < 400f)
		{
			if (base.photonView.isMine && grabbedTarget != null)
			{
				grabbedTarget.GetPhotonView().RPC("netUngrabbed", PhotonTargets.All);
	...
	public void hitAnkleRPC(int viewID)
	{
		...
		PhotonView photonView = PhotonView.Find(viewID);
		if (!(photonView == null) && (photonView.gameObject.transform.position - base.transform.position).sqrMagnitude < 400f)
		{
			if (base.photonView.isMine && grabbedTarget != null)
			{
				grabbedTarget.GetPhotonView().RPC("netUngrabbed", PhotonTargets.All);
	...
	{
		if (((PhotonNetwork.isMasterClient && base.photonView.isMine) || IN_GAME_MAIN_CAMERA.Gametype == GameType.Singleplayer) && GuardianClient.Properties.EndlessTitans.Value && !nonAI)
		{
			object[] randomTitanRespawnPoint = GameHelper.GetRandomTitanRespawnPoint();
			GameObject obj = FengGameManagerMKII.Instance.SpawnTitanRaw((Vector3)randomTitanRespawnPoint[0], (Quaternion)randomTitanRespawnPoint[1]);
			TitanClass type = abnormalType;
			obj.GetComponent<TITAN>().setAbnormalType2(type, type.Equals(TitanClass.Crawler));
		}
	...
	public void titanGetHit(int viewId, int speed)
	{
		...
		if ((float)currentHealth < 0f)
		{
			if (PhotonNetwork.isMasterClient)
			{
				...
				GuardianClient.Gamemodes.CurrentMode.OnTitanKilled(this, photonView.owner, speed);
			}
	...
	public void Suicide()
	{
		...
		if (nonAI)
		{
			fengGame.SendKillInfo(isKillerTitan: false, GuardianClient.Properties.SuicideMessage.Value, isVictimTitan: true, (string)PhotonNetwork.player.customProperties[PhotonPlayerProperty.Name]);
		}
	...
	private void Start()
	{
		...
		{
			Minimap.Instance.TrackGameObjectOnMinimap(base.gameObject, Colors.Orange, trackOrientation: true, depthAboveAll: true);
		}
	...
	private void FindNearestFacingHero()
	{
		...
		foreach (GameObject player in FengGameManagerMKII.Instance.Players)
		{
			...
			if (sqrMagnitude < num)
			{
				...
				float current2 = (0f - Mathf.Atan2(vector.z, vector.x)) * 57.29578f;
				if (Mathf.Abs(0f - Mathf.DeltaAngle(current2, baseGameObjectTransform.rotation.eulerAngles.y - 90f)) < num2)
				{
					gameObject = player;
					num = sqrMagnitude;
				}
		...
		if (gameObject != null && myHero != gameObject)
		{
			myHero = gameObject;
	...
	private void FixedUpdate()
	{
		...
		{
			Vector3 vector7 = targetCheckPt - baseTransform.position;
	...
	{
		if (!TitanChecker.IsLevelSetValid(this, info) && base.photonView.isMine)
		{
			base.photonView.RPC("netSetLevel", PhotonTargets.OthersBuffered, myLevel, myDifficulty, this.skinColor);
			return;
		}
		...
		else
		{
			...
			case TitanState.Idle:
				...
				if (!isAlarm)
				{
					...
					if (abnormalType != TitanClass.Punk && abnormalType != TitanClass.Crawler && num < 0.005f)
					{
						SitDown();
						break;
					}
				...
				if (myDistance < chaseDistance || whoHasTauntMe != null)
				{
					...
					angle = (0f - Mathf.Atan2(vector4.z, vector4.x)) * 57.29578f;
				...
				if (PVPfromCheckPt.state == CheckPointState.Titan)
				{
					...
					{
						GameObject chkPtPrevious = PVPfromCheckPt.chkPtPrevious;
						...
						{
							ToPVPCheckpoint(chkPtPrevious.transform.position, 5 + UnityEngine.Random.Range(0, 10));
							PVPfromCheckPt = chkPtPrevious.GetComponent<PVPcheckPoint>();
						}
					}
				}
				else
			...
			case TitanState.Attack:
				...
				if (attackCheckTimeA != 0f && baseAnimation["attack_" + attackAnimation].normalizedTime >= attackCheckTimeA && baseAnimation["attack_" + attackAnimation].normalizedTime <= attackCheckTimeB)
				{
					if (leftHandAttack)
					{
						GameObject gameObject6 = checkIfHitHand(baseTransform.Find("Amarture/Core/Controller_Body/hip/spine/chest/shoulder_L/upper_arm_L/forearm_L/hand_L/hand_L_001"));
						if (gameObject6 != null)
						{
					...
					{
						GameObject gameObject7 = checkIfHitHand(baseTransform.Find("Amarture/Core/Controller_Body/hip/spine/chest/shoulder_R/upper_arm_R/forearm_R/hand_R/hand_R_001"));
						if (gameObject7 != null)
						{
							Vector3 position5 = baseTransform.Find("Amarture/Core/Controller_Body/hip/spine/chest").position;
							if (IN_GAME_MAIN_CAMERA.Gametype == GameType.Singleplayer)
							{
								gameObject7.GetComponent<HERO>().Die((gameObject7.transform.position - position5) * 15f * myLevel, isBite: false);
							}
							else if (IN_GAME_MAIN_CAMERA.Gametype == GameType.Multiplayer && base.photonView.isMine && !gameObject7.GetComponent<HERO>().HasDied())
							{
								gameObject7.GetComponent<HERO>().MarkDead();
								gameObject7.GetComponent<HERO>().photonView.RPC("netDie", PhotonTargets.All, (gameObject7.transform.position - position5) * 15f * myLevel, false, (!nonAI) ? (-1) : base.photonView.viewID, base.name, true);
							}
						}
					}
				}
				if (!attacked && attackCheckTime != 0f && baseAnimation["attack_" + attackAnimation].normalizedTime >= attackCheckTime)
				{
					...
					GameObject gameObject8 = ((IN_GAME_MAIN_CAMERA.Gametype != GameType.Multiplayer || !base.photonView.isMine) ? ((GameObject)UnityEngine.Object.Instantiate(Resources.Load("FX/" + fxName), fxPosition, fxRotation)) : PhotonNetwork.Instantiate("FX/" + fxName, fxPosition, fxRotation, 0));
					if (nonAI)
					{
						gameObject8.transform.localScale = baseTransform.localScale * 1.5f;
						if (gameObject8.GetComponent<EnemyfxIDcontainer>() != null)
						{
							gameObject8.GetComponent<EnemyfxIDcontainer>().myOwnerViewID = base.photonView.viewID;
						}
					}
					else
					{
						gameObject8.transform.localScale = baseTransform.localScale;
					}
					if (gameObject8.GetComponent<EnemyfxIDcontainer>() != null)
					{
						gameObject8.GetComponent<EnemyfxIDcontainer>().titanName = base.name;
					}
					float b = 1f - Vector3.Distance(currentCamera.transform.position, gameObject8.transform.position) * 0.05f;
					b = Mathf.Min(1f, b);
				...
				if (attackAnimation == "throw")
				{
					...
					if (throwRock != null && baseAnimation["attack_" + attackAnimation].normalizedTime >= 0.62f)
					{
						...
						if (myHero != null)
						{
							...
							float num5 = myHero.transform.position.y + 2f * myLevel - throwRock.transform.position.y;
							vector6 = new Vector3(vector6.x, num5 / num3 - 0.5f * num4 * num3, vector6.z);
						}
						else
						{
				...
				case "crawler_jump_0":
				{
					if (!attacked)
					{
						if (baseAnimation["attack_" + attackAnimation].normalizedTime >= 0.68f)
						{
							...
							{
								float num6 = 120f;
								Vector3 velocity = baseTransform.forward * speed + Vector3.up * num6;
								if (nonAI && abnormalType == TitanClass.Crawler)
								{
									num6 = 100f;
									float a = speed * 2.5f;
									a = Mathf.Min(a, 100f);
									velocity = baseTransform.forward * a + Vector3.up * num6;
								}
							...
							else
							{
								...
								float num10 = y2;
								float num11 = myHero.transform.position.y - y3;
								float num12 = Mathf.Abs((Mathf.Sqrt(num10 * num10 - 4f * num9 * num11) - num10) / (2f * num9));
								Vector3 vector7 = myHero.transform.position + myHero.rigidbody.velocity * num12 + Vector3.up * 0.5f * num7 * num12 * num12;
								float y4 = vector7.y;
								float num13;
								if (num11 < 0f || y4 - y3 < 0f)
								{
									...
									Vector3 velocity2 = baseTransform.forward * a2 + Vector3.up * num13;
									baseRigidBody.velocity = velocity2;
									break;
								}
								float num14 = y4 - y3;
								float num15 = Mathf.Sqrt(2f * num14 / gravity);
								num13 = gravity * num15;
								num13 = Mathf.Max(30f, num13);
								Vector3 vector8 = (vector7 - baseTransform.position) / num12;
								abnorma_jump_bite_horizon_v = new Vector3(vector8.x, 0f, vector8.z);
								Vector3 velocity3 = baseRigidBody.velocity;
								Vector3 force = new Vector3(abnorma_jump_bite_horizon_v.x, velocity3.y, abnorma_jump_bite_horizon_v.z) - velocity3;
								baseRigidBody.AddForce(force, ForceMode.VelocityChange);
								baseRigidBody.AddForce(Vector3.up * num13, ForceMode.VelocityChange);
								float num16 = Vector2.Angle(new Vector2(baseTransform.position.x, baseTransform.position.z), new Vector2(myHero.transform.position.x, myHero.transform.position.z));
								num16 = Mathf.Atan2(myHero.transform.position.x - baseTransform.position.x, myHero.transform.position.z - baseTransform.position.z) * 57.29578f;
								baseGameObjectTransform.rotation = Quaternion.Euler(0f, num16, 0f);
							}
						}
						else
						{
					...
					if (gameObject10 != null)
					{
						...
						if (IN_GAME_MAIN_CAMERA.Gametype == GameType.Singleplayer)
						{
							gameObject10.GetComponent<HERO>().Die((gameObject10.transform.position - position6) * 15f * myLevel, isBite: false);
						}
						else if (IN_GAME_MAIN_CAMERA.Gametype == GameType.Multiplayer && base.photonView.isMine && !gameObject10.GetComponent<HERO>().HasDied())
						{
							gameObject10.GetComponent<HERO>().MarkDead();
							gameObject10.GetComponent<HERO>().photonView.RPC("netDie", PhotonTargets.All, (gameObject10.transform.position - position6) * 15f * myLevel, true, (!nonAI) ? (-1) : base.photonView.viewID, base.name, true);
						}
						if (abnormalType == TitanClass.Crawler)
						{
			...
			case TitanState.Grabbing:
				...
				{
					GameObject gameObject2 = checkIfHitHand(currentGrabHand);
					if (gameObject2 != null)
					{
						if (isGrabHandLeft)
						{
							eatSetL(gameObject2);
							grabbedTarget = gameObject2;
						}
						else
						{
							EatSet(gameObject2);
			...
			case TitanState.Chase:
				...
				else
				{
					...
					{
						Vector3 vector3 = myHero.transform.position - baseTransform.position;
						float current2 = (0f - Mathf.Atan2(vector3.z, vector3.x)) * 57.29578f;
						float f2 = 0f - Mathf.DeltaAngle(current2, baseGameObjectTransform.rotation.eulerAngles.y - 90f);
						if (myDistance < attackDistance * 3f && UnityEngine.Random.Range(0f, 1f) < 0.1f && Mathf.Abs(f2) < 90f && myHero.transform.position.y < neck.position.y + 30f * myLevel && myHero.transform.position.y > neck.position.y + 10f * myLevel)
						{
							Attack2("crawler_jump_0");
							break;
						}
						GameObject gameObject3 = checkIfHitCrawlerMouth(head, 2.2f);
						if (gameObject3 != null)
						{
							Vector3 position = baseTransform.Find("Amarture/Core/Controller_Body/hip/spine/chest").position;
							if (IN_GAME_MAIN_CAMERA.Gametype == GameType.Singleplayer)
							...
							{
								if (gameObject3.GetComponent<TITAN_EREN>() != null)
								{
									gameObject3.GetComponent<TITAN_EREN>().HitByTitan();
								}
								else if (!gameObject3.GetComponent<HERO>().HasDied())
								{
									gameObject3.GetComponent<HERO>().MarkDead();
									gameObject3.GetComponent<HERO>().photonView.RPC("netDie", PhotonTargets.All, (gameObject3.transform.position - position) * 15f * myLevel, true, (!nonAI) ? (-1) : base.photonView.viewID, base.name, true);
								}
							}
						}
			...
			case TitanState.ToCheckPoint:
				...
				if (checkPoints.Count > 0)
				{
					...
					else
					{
						...
						{
							GameHelper.Broadcast("An abnormal titan is approaching the North gate!".AsColor("CC0000").AsBold().AsItalic());
						}
						Vector3 vector5 = (targetCheckPt = (Vector3)checkPoints[0]);
		...
		if (!hasDie)
		{
			...
			if (abnormalType != TitanClass.Crawler)
			{
				...
				if (shouldLookAtTarget && myHero != null && myDistance < 100f)
				{
					...
					float value = 0f - Mathf.DeltaAngle(angle, baseTransform.rotation.eulerAngles.y - 90f);
	...
	public void explode()
	{
		...
		if (IN_GAME_MAIN_CAMERA.Gametype == GameType.Singleplayer)
		{
			UnityEngine.Object.Instantiate(Resources.Load("FX/Thunder"), vector, Quaternion.Euler(270f, 0f, 0f));
			UnityEngine.Object.Instantiate(Resources.Load("FX/boom1"), vector, Quaternion.Euler(270f, 0f, 0f));
		}
		else
		{
		...
		foreach (Renderer renderer in componentsInChildren)
		{
			if (renderer.name.Contains("eye"))
			{
				...
				else
				{
					...
					{
						WWW link3 = SkinChecker.CreateWWW(eye);
						if (link3 != null)
						{
							yield return link3;
							Texture2D mainTexture = RCextensions.LoadImage(link3, flag, 500000);
							link3.Dispose();
							if (!FengGameManagerMKII.LinkHash[0].ContainsKey(eye))
							{
								unload = true;
			...
			{
				if (!(renderer.name == "hair") || (!body.EndsWith(".jpg") && !body.EndsWith(".png") && !body.EndsWith(".jpeg")))
				{
					continue;
				}
				if (!FengGameManagerMKII.LinkHash[2].ContainsKey(body))
				{
					WWW link3 = SkinChecker.CreateWWW(body);
					if (link3 != null)
					{
						yield return link3;
						Texture2D mainTexture2 = RCextensions.LoadImage(link3, flag, 2000000);
						link3.Dispose();
						if (!FengGameManagerMKII.LinkHash[2].ContainsKey(body))
						{
							unload = true;
