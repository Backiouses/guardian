...
public class PhotonView : Photon.MonoBehaviour
{
	...
	protected internal void OnDestroy()
	{
		...
		if (!destroyedByPhotonNetworkOrQuit && !Application.isLoadingLevel)
		{
			...
			{
				Debug.LogError("OnDestroy() seems to be called without PhotonNetwork.Destroy()?! GameObject: " + base.gameObject?.ToString() + " Application.isLoadingLevel: " + Application.isLoadingLevel);
			}
			else if (viewID <= 0)
			{
