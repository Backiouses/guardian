...
public class IN_GAME_MAIN_CAMERA : MonoBehaviour
{
	...
	public void SetLighting(DayLight val)
	{
		...
		GuardianClient.Properties.UseMainLightColor.OnValueChanged();
	...
	public void SetHUDPosition()
	{
		...
		obj.transform.localPosition = new Vector3((int)((float)Screen.width * 0.5f), (int)((float)(-Screen.height) * 0.5f), 0f);
		obj.GetComponent<UILabel>().text = "Pause: " + GameObject.Find("InputManagerController").GetComponent<FengCustomInputs>().inputString[InputCode.Pause] + " ";
		...
		GameObject obj2 = GameObject.Find("Chatroom");
		obj2.transform.localPosition = new Vector3((int)((float)(-Screen.width) * 0.5f), (int)((float)(-Screen.height) * 0.5f), 0f);
		obj2.GetComponent<InRoomChat>().UpdatePosition();
	...
	public void SetSpectorMode(bool val)
	{
		...
		obj.GetComponent<SpectatorMovement>().disable = !val;
		obj.GetComponent<MouseLook>().disable = !val;
	}
	...
	private void MoveCamera()
	{
		...
		float num = (GuardianClient.Properties.UseRawInput.Value ? Input.GetAxisRaw("Mouse X") : Input.GetAxis("Mouse X"));
		float num2 = (GuardianClient.Properties.UseRawInput.Value ? Input.GetAxisRaw("Mouse Y") : Input.GetAxis("Mouse Y"));
		float angle = num * 10f * GetSensitivityMulti();
		float num3 = (0f - num2) * 10f * GetSensitivityMulti() * (float)GetReverse();
		switch (CameraMode)
		{
		case CameraType.WOW:
			if (Input.GetKey(KeyCode.Mouse1))
			{
				...
				float num4 = base.transform.rotation.eulerAngles.x % 360f;
				float num5 = num4 + num3;
				if ((!(num3 > 0f) || ((!(num4 < 260f) || !(num5 > 260f)) && (!(num4 < 80f) || !(num5 > 80f)))) && (!(num3 < 0f) || ((!(num4 > 280f) || !(num5 < 280f)) && (!(num4 > 100f) || !(num5 < 100f)))))
				{
					base.transform.RotateAround(base.transform.position, base.transform.right, num3);
				}
			}
			break;
		case CameraType.TPS:
		{
			...
			base.transform.RotateAround(base.transform.position, Vector3.up, angle);
			float num6 = base.transform.rotation.eulerAngles.x % 360f;
			float num7 = num6 + num3;
			if ((!(num3 > 0f) || ((!(num6 < 260f) || !(num7 > 260f)) && (!(num6 < 80f) || !(num7 > 80f)))) && (!(num3 < 0f) || ((!(num6 > 280f) || !(num7 < 280f)) && (!(num6 > 100f) || !(num7 < 100f)))))
			{
				base.transform.RotateAround(base.transform.position, base.transform.right, num3);
			}
			break;
		...
		case CameraType.Original:
		{
			...
			{
				angle = (0f - ((float)Screen.width * 0.4f - mousePosition.x) / (float)Screen.width * 0.4f) * GetSensitivityMultiWithTimeDelta() * 150f;
				base.transform.RotateAround(base.transform.position, Vector3.up, angle);
			}
			else if (mousePosition.x > (float)Screen.width * 0.6f)
			{
				angle = (mousePosition.x - (float)Screen.width * 0.6f) / (float)Screen.width * 0.4f * GetSensitivityMultiWithTimeDelta() * 150f;
				base.transform.RotateAround(base.transform.position, Vector3.up, angle);
			}
			num3 = 140f * ((float)Screen.height * 0.6f - mousePosition.y) / (float)Screen.height * 0.5f;
			base.transform.rotation = Quaternion.Euler(num3, base.transform.rotation.eulerAngles.y, base.transform.rotation.eulerAngles.z);
			break;
		}
		}
		if (GuardianClient.Properties.FPSCamera.Value)
		{
			base.transform.position = ((head == null) ? main_object.transform.position : head.transform.position);
	...
	private GameObject FindNearestTitan()
	{
		...
		{
			float magnitude = (allTitan.transform.Find("Amarture/Core/Controller_Body/hip/spine/chest/neck").position - position).magnitude;
			FEMALE_TITAN component = allTitan.GetComponent<FEMALE_TITAN>();
			if (!(component != null) || !component.hasDie)
			{
				TITAN component2 = allTitan.GetComponent<TITAN>();
				if ((!(component2 != null) || !component2.hasDie) && magnitude < num)
				{
					result = allTitan.gameObject;
	...
	public void Snapshot2(int index)
	{
		...
		if (snapShotTarget != null && (bool)snapShotTarget.GetComponent<TITAN>())
		{
			float num2 = num;
			float num3 = index - 1;
			num = num2 + num3 * snapShotTarget.transform.localScale.x * 10f;
		}
		...
		if (index == 1)
		{
			UITexture component = GameObject.Find("UI_IN_GAME").GetComponent<UIReferArray>().panels[0].transform.Find("snapshot1").GetComponent<UITexture>();
			component.mainTexture = snapshot1;
			component.transform.localScale = new Vector3((float)Screen.width * 0.4f, (float)Screen.height * 0.4f, 1f);
			component.transform.localPosition = new Vector3((float)(-Screen.width) * 0.225f, (float)Screen.height * 0.225f, 0f);
			component.transform.rotation = Quaternion.Euler(0f, 0f, 10f);
	...
	{
		if (!int.TryParse((string)FengGameManagerMKII.Settings[95], out var result) || damage >= result)
		{
			snapShotCount = 1;
	...
	private void CreateMinimap()
	{
		if (FengGameManagerMKII.Level != null)
		{
			...
			if (Minimap.Instance.myCam == null)
			{
				...
				gameObject = (GameObject)Object.Instantiate(gameObject, Vector3.zero, Quaternion.identity);
			}
