...
public static class PhotonNetwork
{
	...
	public static bool connected
	{
		get
		{
			...
			if (!networkingPeer.IsInitialConnect && networkingPeer.State != PeerState.PeerCreated && networkingPeer.State != PeerState.Disconnected && networkingPeer.State != PeerState.Disconnecting)
			{
				return networkingPeer.State != PeerState.ConnectingToNameServer;
			}
			return false;
		}
	}
	...
	static PhotonNetwork()
	{
		...
		networkingPeer = new NetworkingPeer(photonMono, string.Empty, NetworkHelper.Connection.Protocol);
