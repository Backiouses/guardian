...
public class CustomCharacterManager : MonoBehaviour
{
	...
	public void OnHairRChange(float value)
	{
		if (setup.myCostume != null && setup.part_hair != null)
		{
			...
			float g = setup.part_hair.renderer.material.color.g;
			myCostume.hair_color = new Color(value, g, setup.part_hair.renderer.material.color.b);
			SetHairColor();
		}
	}
	...
	public void OnHairGChange(float value)
	{
		...
		{
			HeroCostume myCostume = setup.myCostume;
			float r = setup.part_hair.renderer.material.color.r;
			myCostume.hair_color = new Color(r, value, setup.part_hair.renderer.material.color.b);
			SetHairColor();
		}
	}
	...
	public void OnHairBChange(float value)
	{
		if (setup != null && setup.myCostume != null && setup.part_hair != null)
		{
			HeroCostume myCostume = setup.myCostume;
			float r = setup.part_hair.renderer.material.color.r;
			myCostume.hair_color = new Color(r, setup.part_hair.renderer.material.color.g, value);
			SetHairColor();
		}
	}
	...
	private void RefreshLabel()
	{
		...
		labelPOINT.GetComponent<UILabel>().text = "Points: " + (400 - CalculateStatPoints());
		labelSPD.GetComponent<UILabel>().text = "SPD " + setup.myCostume.stat.Speed;
		labelGAS.GetComponent<UILabel>().text = "GAS " + setup.myCostume.stat.Gas;
		labelBLA.GetComponent<UILabel>().text = "BLA " + setup.myCostume.stat.Blade;
		labelACL.GetComponent<UILabel>().text = "ACL " + setup.myCostume.stat.Accel;
	...
	private int CalculateStatPoints()
	{
		...
		{
			return 0 + setup.myCostume.stat.Speed + setup.myCostume.stat.Gas + setup.myCostume.stat.Blade + setup.myCostume.stat.Accel;
		}
		return 400;
	}
